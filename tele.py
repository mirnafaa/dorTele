...
async def process_fore_check(phone_root: str, pin: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            access_token=token_data.get('payload',{}).get('access_token');
            ...
            return{"success":True,"nama":nama,"reff":reff,"total_points":total_poin,"vouchers":vouchers_list}
...
async def receive_fore_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    phone_input = update.message.text; phone_cleaned = re.sub(r"[^0-9]", "", phone_input);
...
async def receive_fore_pin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not (pin_input.isdigit() and len(pin_input) == 6): await update.message.reply_text("PIN salah format. Coba lagi:"); return ASK_FORE_PIN
    ...
    else: await update.message.reply_text(f"Gagal: {data.get('message', 'Error')}", reply_markup=markup)
...
async def receive_order_pin_and_get_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    pin_input = update.message.text; phone = context.user_data.get("phone_root"); user_id = update.effective_user.id
    markup = admin_reply_markup if is_admin_check(user_id) else user_reply_markup
    if not (pin_input.isdigit() and len(pin_input) == 6): await update.message.reply_text("PIN salah format. Coba lagi:"); return ASK_ORDER_PIN
    ...
    if not orders_result.get("success"): await update.message.reply_text(f"Gagal ambil order: {orders_result.get('message')}", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    ongoing_orders = orders_result.get("data", [])
    if not ongoing_orders: await update.message.reply_text("Tidak ada orderan berjalan.", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    ...
    await update.message.reply_text("Pilih orderan:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def select_order_and_show_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    except Exception: await update.callback_query.edit_message_text("Data salah. Dibatalkan.", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    ...
    if not all([access_token, refresh_token, device_id]): await update.callback_query.edit_message_text("Sesi habis. Ulangi.", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    ...
    if not detail_result.get("success"): await context.bot.send_message(chat_id=user_id, text=f"Gagal ambil detail: {detail_result.get('message')}", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    data = detail_result.get("data", {})
    if not data: await context.bot.send_message(chat_id=user_id, text="Gagal dapat detail.", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
    try:
        status = "Dalam Pembuatan 👨‍🍳" if data.get('uor_status') == 'in_process' else "Ready For PickUp 픽업 준비 완료 🥤"
        antrian = data.get('uor_queue', 'N/A'); receipt_url = data.get('url_webview_e_receipt', 'N/A'); qr_hash = data.get('uorsh_hash')
        hasil_teks = (f"✅ **Order Ditemukan!**\n\nNo Antrian: **{antrian}**\nStatus: **{status}**\n\nE-Receipt: [Klik di sini]({receipt_url})")
        cost = 5
        if not is_admin_check(user_id):
            try: update_credits(user_id, -cost); logger.info(f"{cost} kredit dipotong (Cek Order)")
            except Exception as e: logger.error(f"Gagal potong kredit {user_id}: {e}"); await context.bot.send_message(chat_id=user_id, text="Error potong kredit.", reply_markup=markup); context.user_data.clear(); return ConversationHandler.END
        if not qr_hash: await context.bot.send_message(chat_id=user_id, text=hasil_teks, parse_mode='Markdown', reply_markup=markup, disable_web_page_preview=True)
        else:
            qr = qrcode.QRCode(version=1, box_size=10, border=4); qr.add_data(qr_hash); qr.make(fit=True)
            img = qr.make_image(fill_color="black", back_color="white"); bio = io.BytesIO(); img.save(bio, 'PNG'); bio.seek(0)
    ...
    except Exception as e: logger.error(f"Error proses detail order: {e}"); await context.bot.send_message(chat_id=user_id, text=f"Error proses data: {e}", reply_markup=markup)
    context.user_data.clear(); return ConversationHandler.END
...
def main() -> None:
    ...
    order_conv_handler = ConversationHandler(entry_points=[MessageHandler(filters.TEXT & filters.Regex("^Cek Orderan$"), start_order_check)], states={ASK_ORDER_PHONE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_order_phone)], ASK_ORDER_PIN: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_order_pin_and_get_list)], SELECTING_ORDER: [CallbackQueryHandler(select_order_and_show_detail, pattern="^order_")],}, fallbacks=[CommandHandler("cancel", cancel_order_check), CallbackQueryHandler(select_order_and_show_detail, pattern="^order_cancel$")], per_user=True, per_message=False)
